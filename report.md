...
| `Meeting` | 모임 정보 (카테고리, 제목, 내용, 위치 JSON, 일정, 최대 인원) |
...
| 모임 목록 (`/meetings/`) | 카테고리 필터 탭 + 모임 제목 기반 카드 그리드 |
| 모임 생성 (`/meetings/create/`) | 카테고리, 제목, 내용 작성 + 카카오맵 + 달력 |
| 채팅방 (`/meetings/<id>/chat/`) | 모임 제목 뱃지 + 참여자 목록 + 실시간 메시지 + 평가/신고 버튼 |
